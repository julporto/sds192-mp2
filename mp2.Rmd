...
The data on contributions to candidates comes from the Federal Election Committee. The units are in dollars. The data on the population of each state comes from census data of the same year (https://www.census.gov/data/datasets/2017/demo/popest/state-total.html).
...
We can see from the graphs above that North Carolina (NC) has by far the most contributions per person, and Minnesota (MN) the least. 

The difference between the most contributing state NC and the following state Georgia (GA) is quite big- almost 500 dollars, but the gap between the least contributing state Minnesota (MN) and the next-to-last contributing state South Dakota (SD), is very small- less than 5 dollars. 
